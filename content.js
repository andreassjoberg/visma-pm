  ...
  let result = projects.map(project => {
    ...
    let employeeResult = employees.map(employee => {
      ...
      let employeeArticles = articles.map(article => {
        ...
        return `\t\t${article}\t${articlesTime.toLocaleString("sv-SE", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}\n${comments}`;
      });

      return `\t${employee}\t\t${
        employeeTime.toLocaleString("sv-SE", { minimumFractionDigits: 2, maximumFractionDigits: 2 })
      }\n${
        employeeArticles.reduce((prev, current) => prev += `${current}\n`, "")
      }`;
    });

    return employeeResult.reduce((prev, current) => prev += `${project}\n${current}\n`, "");
  });
...
function formatItem(item) {
  return `${item.employeeName}\t${item.invoiceText}\t${item.invoiceHours.toLocaleString("sv-SE", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
}
...
function addOwnSection() {
  ...
  if (sections.length > 0) {
    ...
    node.id = divId;
    node.className = "col-md-10 margin-top";
    sections[0].prepend(node);
    return true;
  }
  return false;
}
